...
#if !UNITY_WINRT && !UNITY_IOS && !UNITY_ANDROID
using System.Xml;
#endif
...
{
	public static string m_version = "v2.7";
	...
	public List<LetterAnimation> LetterAnimations { get { return m_master_animations; } }
    ...
    {	
		...
		if(m_master_animations != null)
		{
			bool imported_legacy_effect_setup = false;
			...
			{
				for(int action_idx=0; action_idx < animation.NumActions; action_idx++)
				{
	...
	public void SetText(string new_text, bool force_all_new = false)
	{
		if(m_renderer == null)
		{
			...
			//Debug.Log("m_renderer.sharedMaterial.renderQueue = " + m_renderer.sharedMaterial.renderQueue);
			m_renderer.sharedMaterial.renderQueue = 3210;
		}
		...
		do
		{
			...
			foreach(LetterSetup letter in m_letters)
			{
                if(letter != null && !letter.m_base_offsets_setup)
				{
	...
	void OnDestroy()
	{
		...
		if(m_letters != null)
		{
			foreach(LetterSetup letter in m_letters)
			{
				if(Application.isPlaying)
				{
                    if(letter != null && letter.m_mesh != null)
	...
	public void PlayAudioClip(AudioEffectSetup effect_setup, AnimationProgressionVariables progression_vars, AnimatePerOptions animate_per)
	{
        ...
        if( Core.Data != null && Core.Data.usrManager.UserConfig.mute == 0) {
            PlayClip(
                source,
	...
	public string ExportData(bool hard_copy = false)
	{
		...
		if(hard_copy)
		{
			...
			json_data["m_line_height"] = m_line_height;
...
//			json_data["m_text"] = m_text;
	...
	public void ImportData(string data, bool force_clear_old_audio_particles = false)
	{
		...
		if(json_data != null)
		{
			...
			if(json_data.ContainsKey("m_line_height")) m_line_height = (float) json_data["m_line_height"].Number;
