...
//		DrawDefaultInspector();
		...
		{
			font_manager.gameObject.renderer.material = font_manager.m_font.material;
		...
		if(GUI.changed && font_manager.m_font_data_file != null && font_manager.m_font_material != null)
		{
			...
			font_manager.gameObject.renderer.material = font_manager.m_font_material;
			font_manager.SetText(font_manager.m_text, true);
			return;
		}
	...
	void OnSceneGUI()
	{
		...
		for(int anim_idx=0; anim_idx < font_manager.NumAnimations; anim_idx++)
		{
			...
			for(int idx=0; idx < letter_animation.NumActions; idx++)
			{
				...
				if(action.m_start_pos.Progression == ActionPositionVector3Progression.CURVE_OPTION_INDEX && action.m_start_pos.m_bezier_curve.EditorVisible)
				{
					action.m_start_pos.m_bezier_curve.OnSceneGUI(position_offset, font_manager.Scale);
				...
				if(action.m_end_pos.Progression == ActionPositionVector3Progression.CURVE_OPTION_INDEX && action.m_end_pos.m_bezier_curve.EditorVisible)
				{
					action.m_end_pos.m_bezier_curve.OnSceneGUI(position_offset, font_manager.Scale);
