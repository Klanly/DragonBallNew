...
public class BanCameraController : MonoBehaviour
{
	...
	//定义全局存在标记
	public static BanCameraController Instance;
	...
	void Update()
	{
		...
		else
		{
			...
			foreach (GameObject aGo in go_Objects)
			{
                ...
                if (left > aGo.renderer.bounds.center.z - aGo.renderer.bounds.extents.z)
				{
					//左边记录z最小值
                    left = aGo.renderer.bounds.center.z - aGo.renderer.bounds.extents.z;
				}

				//如果左边 小于  当前网格模型中心坐标.z + 当前网格宽度.z 
                if (right < aGo.renderer.bounds.center.z + aGo.renderer.bounds.extents.z)
				{
					//右边记录z最大值
                    right = aGo.renderer.bounds.center.z + aGo.renderer.bounds.extents.z;
				}
            ...
            Vector3 targetPos = new Vector3(distance, (up + down) / 2, (left + right) / 2);
    		//如果新定位高度小于最小高度
