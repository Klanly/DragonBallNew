...
public class NewUIGuide : MonoBehaviour {
	
	public static NewUIGuide _Instance;
...
//	public GameObject go_BuMa4_Dia;
	...
	public List<UISprite> LIST_ROLE = new List<UISprite>();
	public List<GameObject> LIST_MASK = new List<GameObject>();
	public List<ChatLabel> LIST_Dialogue = new List<ChatLabel>();
...
//	}
	...
	public static NewUIGuide Instance
	{
		get
		{
			if(_Instance == null)
			{
				Object prefab = PrefabLoader.loadFromPack("JC/UI Root_NewGuide");
				if(prefab != null)
				{
					...
					if(obj != null)
					{
						...
						_Instance = obj.GetComponentInChildren<NewUIGuide>();
...
//				else
	...
	void SetRoleType(GuideData data)
	{
		...
		{
			if(!LeftNPC.activeSelf)LeftNPC.SetActive(true);
			if(RightNPC.activeSelf)RightNPC.SetActive(false);
			LIST_Dialogue[0].SetText("     " + data.Dialogue);
			ShowRoleIndex = 0;
		}
		else if(data.RoleDir == 1)    //对话人物(右)
		{
			if(!RightNPC.activeSelf)RightNPC.SetActive(true);
			if(LeftNPC.activeSelf)LeftNPC.SetActive(false);
