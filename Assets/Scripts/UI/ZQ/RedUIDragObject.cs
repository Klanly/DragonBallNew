...
public class RedUIDragObject : MonoBehaviour
{
    ...
    public Vector2 limitH = Vector2.zero;
    [SerializeField] protected Vector2 limit_h = Vector2.zero;
    public Vector2 limitV = Vector2.zero;
    [SerializeField] protected Vector2 limit_v = Vector2.zero;
    ...
    void Move (Vector3 worldDelta)
    {
        ...
        if(mPanel != null){
            if (mTargetPos.y > limitV.y)
            {
                mTargetPos.y = limitV.y;
                target.localPosition = mTargetPos;
                //   target.GetComponent<PVEDownloadCartoonController>().OpenCartoonPanel();
            }
            //  target.localPosition = mTargetPos;
    ...
    /*  void Move (Vector3 worldDelta)
    {
        ...
        if (mPanel != null)
        {
            ...
            if(limit_h != Vector2.zero )
            {
                if(mTargetPos.x > limit_h.x)
                {
                    mTargetPos.x = limit_h.x;
                }
                if(mTargetPos.x < limit_h.y)
                {
                    mTargetPos.x = limit_h.y;
                }
            }

            if(limit_v != Vector2.zero)
            {
                if(mTargetPos.y > limit_v.x)
                {
                    mTargetPos.y = limit_v.x;
                }

                if(mTargetPos.y < limit_v.y)
                {
                    mTargetPos.y = limit_v.y;
                }
            }
            ...
            //added by zhangqiang at 2014-3-12 for dargged in range
        ...
        {
            if(mPressed && !m_bIsHiding)
            {
                m_bIsShowing = false;
                m_bIsHiding = true;
                if(SQYMainController.mInstance != null)
                {
                    SQYMainController.mInstance.willHideUI();
                }
                if(DBUIController.mDBUIInstance._playerViewCtl != null)
                {
                    DBUIController.mDBUIInstance._playerViewCtl.WillHideUI();
                }

            }
            else if(!mPressed && !m_bIsShowing)
            {
                m_bIsHiding = false;
                m_bIsShowing = true;
                if(SQYMainController.mInstance != null)
                {
                    SQYMainController.mInstance.willShowUI();
                }
                if(DBUIController.mDBUIInstance._playerViewCtl != null)
                {
                    DBUIController.mDBUIInstance._playerViewCtl.WillShowUI();
                }
...
//    float target_up_y =0;
//    float target_down_y = -580;
    float limit_y = -100;

    public void AutoMovePos(){
...
//            target_down_y = -580;
            ...
            if (limit_y >= target_y)
            {
                target.GetComponent<PVEDownloadCartoonController>().CloseCartoonPanel();
            }
            else
            {
                target.GetComponent<PVEDownloadCartoonController>().OpenCartoonPanel();
