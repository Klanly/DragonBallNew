...
namespace SocketIOClient.Messages
{
    public class JSONMessage : Message
    {
        ...
        {
			this.MessageText = JsonFx.Json.JsonWriter.Serialize(value);
        }

        public virtual T Message<T>()
        {
			try { return JsonFx.Json.JsonReader.Deserialize<T>(this.MessageText); }
            catch (Exception ex)
            {
		 ...
		 public JSONMessage(object jsonObject):this()
        {
			...
			this.MessageText = JsonFx.Json.JsonWriter.Serialize(jsonObject );
        }
