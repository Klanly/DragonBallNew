...
namespace AW.Resources {
    ...
    public abstract class IAssetManager  {
        ...
        // value 是应用计数
        protected LRUCache<string, int> lstRefAsset = null;
        ...
        protected IAssetManager (int capacity, int max) {
            lstRefAsset = new LRUCache<string, int>(capacity, max);
        }
    }
    ...
    public class ResourceManager {
		...
		private int errorRepeat = 1;

        private Dictionary<ManagerType, IAssetManager> AMgr = null;

        public ResourceManager () {
            dicAsset = new Dictionary<string, UObj>();
            dicLoadingReq = new Dictionary<string, WWW>();
            AMgr = new Dictionary<ManagerType, IAssetManager>();
...
//                    ConsoleEx.DebugLog("<GetResource Failed> Res not exist, res.Name = " + ABname);
        ...
        public void LoadAsync(AssetTask task) {
            ...
            if(dicLoadingReq.Count < MAX_WWW_COUNT) {
                if(Core.SM.rtPlat == RuntimePlatform.OSXEditor || Core.SM.rtPlat == RuntimePlatform.OSXPlayer) {
            ...
            if(!errorOcurred)
			{
				...
				else 
				{
					...
					if (!errorOcurred) {
						...
						TaskEnd = false;	
//						SouceData tData = new SouceData (assetBundleName,1,Core.Data.sourceManager.getSouceSize(assetBundleName));
//						tData.isError = 0;
					}
                }
				...
				if (TaskEnd) {
					...
					if (UIDownloadPacksWindow.m_Instance != null) {
						UIDownloadPacksWindow.m_Instance.ForceJumpEnd (task);
					}
				}
            }
